...
.state('sachphathanh',{
        url: '',
        templateUrl:'MainIndex/sapphathanh.html',
...
.state('sachphathanhchitiet',{
        url: '/Sắp phát hành/:id',
...
.state('vanhocchitiet',{
    	...
    	app.factory('SachsFactory1', function($resource) {
 		 return $resource('data/sachsapphathanh1.json');
			});

			app.controller('productlistCtrl', function ($scope, SachsFactory1) {
			  $scope.sachphathanhs = SachsFactory1.query();

			  $scope.itemsPerPage = 12
			  $scope.currentPage = 1;

			  $scope.pageCount = function () {
			    return Math.ceil($scope.sachphathanhs.length / $scope.itemsPerPage);
			  };

			  $scope.sachphathanhs.$promise.then(function () {
			    $scope.$watch('currentPage + itemsPerPage', function() {
			      var begin = (($scope.currentPage - 1) * $scope.itemsPerPage),
			        end = begin + $scope.itemsPerPage;

			      $scope.filteredSachs = $scope.sachphathanhs.slice(begin, end);
			    });
			  });
			});
