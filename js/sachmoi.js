...
app.factory('SachsFactory1', function($resource) {
     return $resource('data/sachmoi1.json');
      });
...
.state('sachmoi',{
        url: '',
        templateUrl:'MainIndex/sachmoi.html',
			...
			app.controller('productlistCtrl', function ($scope, SachsFactory1) {
			  ...
			  $scope.sachmois.$promise.then(function () {
			    $scope.$watch('currentPage + itemsPerPage', function() {
			      ...
			      $scope.filteredSachs = $scope.sachmois.slice(begin, end);
			    });
			  });
			});
...
app.controller('productCtrl', function($stateParams,$scope,SachsFactory1) {
  $scope.sachmois = SachsFactory1.query();
